...
export default {
  ...
  methods: {
    ...
    calculate (parameter) {
      ...
      if (!isEmpty(queryParam)) {
        ...
        data = data.filter(u => {
            Object.keys(queryParam).every(i => {
                console.log(deepGet(u, i))
                console.log(queryParam[i])
                const str = deepGet(u, i)
                return `${deepGet(u, i)}`.indexOf(queryParam[i]) !== -1
            })
        })
        this.localPagination = this.showPagination && Object.assign({}, this.localPagination, {
          total: data.length
    ...
    //   if (!isEmpty(queryParam)) {
    //       if (queryParam.CreateTime && queryParam.CreateTime[0] !== 'NaN') {
    //        data = data.filter(u => { if (+queryParam.CreateTime[1] > +u.CreateTime && +u.CreateTime > +queryParam.CreateTime[0]) { return u } })
    //        if (queryParam.TableName) {
    //             const { CreateTime, ...query } = queryParam
    //             data = data.filter(u => Object.keys(query).every(i => `${deepGet(u, i)}`.indexOf(query[i]) !== -1))
    //        }
    //       } else {
  ...
  render () {
    ...
    this.$scopedSlots = Object.assign({}, {
      time: text => convert(text.toString(), 'unix')
    }, this.$scopedSlots)
    return (
