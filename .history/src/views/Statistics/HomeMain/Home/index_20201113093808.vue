        ...
        <a-row :gutter="48">
          <a-col
            ...
            <a-form-item label="区县">
              <a-select placeholder="请选择区县" v-model="queryParam.code" allowClear >
...
export default {
  ...
  data () {
    ...
    return {
      ...
      loadData: async (parameter) => {
        if (this.queryParam.start_time === undefined || typeof (this.queryParam.start_time) === 'number') {
        ...
        return {
            data: this.result,
            queryParam: this.queryParam.code
         }
      }
    }
  },
  methods: {
    toDistrict (row) {
      ...
      this.$router.push({ path: '/Statistics/HomeMain/District', query: { code: row.code, start_time: this.queryParam.start_time } })
    },
    ErrorLog (row) {
        if (row) {
            row.is_town = true
