  ...
  data () {
    return {
      ...
      loadData: async (parameter) => {
            this.timestamp = parseInt(new Date().getTime() / 1000) - 24 * 60 * 60
            return Getres(Object.assign(parameter, this.queryParam)).then((res) => {
            })
        //   res.result.data.forEach(u => {
        //         this.result.push(u.District)
        //   })
        //   return res.result
        // })
            // const { data } = await Getres({ start_time: +timestamp })
            // this.result = data
            // return {
            //     data: this.result,
