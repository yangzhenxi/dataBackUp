...
export default {
  ...
  data () {
    ...
    return {
      ...
      columns: [
        {
          ...
          customRender: (val, row, index) => {
            const obj = {
              children: this.District[index],
              attrs: {}
            }
            obj.attrs.rowSpan = this.getRowSpanCount(this.District, index)
            return obj
          }
        },
        {
      ...
      loadData: async (parameter) => {
            console.log(this.queryParam.timestamp)
            if (this.queryParam.timestamp === undefined) {
                this.queryParszam.timestamp = parseInt(new Date().getTime() / 1000) - 24 * 60 * 60
            }
            // if (!this.queryParam.timestamp) {
            // } else {
  ...
  methods: {
    ...
    disabledDateTime () {
        console.log(this.range(0, 24).splice(4, 20))
        console.log(this.range(30, 60))
        console.log(this.range(0, 24).splice(4, 20))
      return {
