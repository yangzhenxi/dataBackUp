...
export default {
    ...
    data () {
        return {
			...
			table_data_type: this.$route.query.table_data_type || 'GetReferralres',
            temp: {},
            itemList: [],
			Region: [],
			MomentDate: [],
            ...
            loadData: async (parameter) => {
				...
				this.table_data_type === 'Getres' ? result = await Getres(this.queryParam) : result = await GetReferralres(this.queryParam)
				this.result = []
    ...
    methods: {
        toDistrict (row) {
            ...
            this.$router.push({ path: '/Referral/Statistics/District', query: obj })
        },
        ErrorLog (row) {
			const obj = {
				code: row.code,
				...
				end_time: this.queryParam.end_time,
				is_town: 'true',
				router: 'Referral/Statistics/Home'
			}
