    ...
    <a-spin :spinning="loading">
      <a-form :form="form">
        <a-form-item
          ...
          <a-input
            placeholder="请输入用户名"
            v-decorator="['username', { rules: [{required: true, message: '请输入用户名！'},{validator}],firstFields:true
            }]" />
        </a-form-item>
        <a-form-item
          label="角色"
...
import { nameRepeatspecialValidator } from '@/utils/validator'
import { UpdataUserInfo, GetUserList } from '@/api/system/user'
export default {
    ...
    methods: {
        async Edit (record) {
            ...
            this.$nextTick(() => {
                setTimeout(() => {
                    this.form.setFieldsValue({ username: record.UserName, role: record.Role })
                })
            })
            this.validatorName = this.deepGet(await GetUserList(), 'data', [])
        },
        handleSubmit () {
            this.form.validateFields(async (errors, values) => {
                ...
                if (!errors) {
                    try {
                        ...
                        UpdataUserInfo(values).then(res => {
