        ...
        <a-row :gutter="48">
          ...
          <a-col :md="12" :sm="24">
            <a-form-item label="时间">
              <a-date-picker v-model="queryParam.start_time" :disabled-date="disabledStartDate" placeholder="请选择开始时间" />
              <span> ---- </span>
              <a-date-picker v-model="queryParam.end_time" :disabled-date="disabledEndDate" placeholder="请选择结束时间" />
            </a-form-item>
          </a-col>
          <a-col
            :md="6"
...
import { convert, deepGet, getRowSpanCount } from '@/utils/util'
let that = Object
export default {
	name: 'TableList',
	mixins: [mixinTable],
    ...
    data () {
        return {
            ...
            columns: [
                {
                    ...
                    customRender: (val, row, index) => {
                        ...
                        const obj = {
                            children: child,
                            attrs: {
                                rowSpan: getRowSpanCount(that.District, index)
                            }
                        }
                        return obj
                    }
            ...
            loadData: async (parameter) => {
				...
				const result = deepGet(await GetPullRes(this.queryParam), 'data', [])
                result.forEach((u, o) => {
    ...
    methods: {
        toDistrict (row) {
            this.$router.push({
                path: `/Statistics/SignUp/pull/${row.code}`,
        ...
        ErrorLog (row) {
			...
			this.queryParam.router = 'Statistics/SignUp/pull'
			this.queryParam.is_town = 'true'
            this.$router.push({ path: '/error/Extractlogs', query: this.queryParam })
        },
        ExportExcel () {
			const itemList = []
