<template>
  <div>
    <a-card>
          ...
          <a-row :gutter="48">
            ...
            <a-col :md="8" :sm="24">
              <a-form-item label="数据筛选">
                <a-select placeholder="请选择要筛选的数据" v-model="queryParam.data_filter" allowClear>
      ...
      <m-table ref="table" size="default" bordered :columns="columns" :data="loadData">
        ...
        <template slot="main_nums" slot-scope="text,row">
          <a-tag color="#f50" v-if="row.OFF">{{ text }}</a-tag>
          <span v-else>{{ text }}</span>
        </template>
        <template slot="backup_nums" slot-scope="text,row">
          <a-tag color="#F50" v-if="row.OFF">{{ text }}</a-tag>
          <span v-else>{{ text }}</span>
        </template>
        <template slot="pull_nums" slot-scope="text,row">
          <div class="flex">
            <a-tag color="#F50" v-if="row.OFF">{{ text }}</a-tag>
            <span v-else>{{ text }}</span>
            <a-button v-show="row.pull_error!==0" type="primary" size="small" @click="ErrorLog(row,'Extractlog')">查看错误日志</a-button>
          </div>
        </template>
        <template slot="upload_nums" slot-scope="text,row">
          <div class="flex">
            <a-tag color="#F50" v-if="row.OFF">{{ text }}</a-tag>
            <span v-else>{{ text }}</span>
            <a-button v-show="row.upload_error!==0" type="primary" size="small" @click="ErrorLog(row,'statisticslog')">查看错误日志 {{ row.upload_error }}个</a-button>
          </div>
        </template>
      </m-table>
    </a-card>
    ...
    //   },
      ...
      columns: [
        ...
        },
        {
			...
			align: 'center',
			dataIndex: 'main_nums',
            scopedSlots: { customRender: 'main_nums' }
        },
        {
			title: '备库',
			align: 'center',
			dataIndex: 'backup_nums',
            scopedSlots: { customRender: 'backup_nums' }
        },
        {
			title: '中间库',
			align: 'center',
			dataIndex: 'pull_nums',
            scopedSlots: { customRender: 'pull_nums' }
        },
        {
			title: '联众库',
			align: 'center',
			dataIndex: 'upload_nums',
            scopedSlots: { customRender: 'upload_nums' }
		}
      ],
      loadData: async () => {
		const destrict = []
		...
		const main = deepGet(data, ['main', 'data'], [])
        const backup = deepGet(data, ['backup', 'data'], [])
        const pull = deepGet(data, ['pull', 'data'], [])
        const upload = deepGet(data, ['upload', 'data'], [])
		...
		const arr = [obj.pull_nums, obj.upload_nums, obj.main_nums, obj.backup_nums]
		if (new Set(arr).size !== 1) {
			obj.OFF = true
		}
        if (this.queryParam.data_filter && this.queryParam.data_filter !== 'All') {
            if (this.queryParam.data_filter === 'On') {
              if (new Set(arr).size === 1) {
				...
				destrict.push(this.District[index])
              }
            } else if (this.queryParam.data_filter === 'OFF') {
              if (new Set(arr).size !== 1) {
				uploadCount = uploadCount + uploadRes[index].upload
				pullCount = pullCount + pullRes[index].pull
				mianCount = mianCount + deepGet(mainRes, [index, 'nums'], pullRes[index].pull)
		...
		destrict.length !== 0 ? this.District = destrict : this.District = this.District
        return {
          data: this.result
        }
      }
